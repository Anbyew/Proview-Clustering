...
color_iter = itertools.cycle(['#e6194b', '#3cb44b', '#ffe119','#0082c8', '#f58231', '#911eb4', '#46f0f0',
                              '#f032e6', '#d2f53c', '#fabebe', '#008080', '#e6beff'])
splot = plt.subplot(2, 1, 2)
Y_ = best_gmm.predict(X[indices])
#visu = PCA(n_components=2).fit_transform(X[indices])
visu = TSNE(n_components=2).fit_transform(X[indices])
...
color_iter = itertools.cycle(['#e6194b', '#3cb44b', '#ffe119','#0082c8', '#f58231', '#911eb4', '#46f0f0',
                              '#f032e6', '#d2f53c', '#fabebe', '#008080', '#e6beff'])
splot = plt.subplot(2, 1, 2)
Y_ = best_gmm_a.predict(X[indices])
#visu = PCA(n_components=2).fit_transform(X[indices])
visu = TSNE(n_components=2).fit_transform(X[indices])
for i, (mean, cov, color) in enumerate(zip(best_gmm_a.means_, best_gmm_a.covariances_,
                                           color_iter)):
    v, w = linalg.eigh(cov)
    if not np.any(Y_ == i):
        continue
